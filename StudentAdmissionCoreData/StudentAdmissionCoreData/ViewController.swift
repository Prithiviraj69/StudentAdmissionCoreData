        ...
        else
        {
            for i in 0..<stdcnt
            {
                if (username.text! == studArray[i].spid) && (password.text! == studArray[i].password)
                {
                    ...
                    UserDefaults.standard.setValue(studArray[i].phoneno, forKey: "mobileno")
                    navigationController?.pushViewController(vc, animated: true)
                    break
                }
                else
                {
                    let alert = UIAlertController(title: "Please Enter Correct username or password", message: "Incorrect Username Or Password", preferredStyle: .alert)
